        ...
        <div>
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-4 mb-5 space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                    ...
                    <Select value={collection} onChange={(e) => updateParams({ collection: e.target.value })}>
                        <option value="">All Collections</option>
                        {collections.map((c) => <option key={c._id} value={c.slug}>{c.name}</option>)}
                    </Select>
                    <Select value={isAvailable} onChange={(e) => updateParams({ isAvailable: e.target.value })}>
                        <option value="">Availability</option>
                        <option value="true">Available</option>
                        <option value="false">Unavailable</option>
                    </Select>
                    <Select value={isFeatured} onChange={(e) => updateParams({ isFeatured: e.target.value })}>
                        <option value="">Featured</option>
                        <option value="true">Featured Only</option>
                    </Select>
                    <Select value={isNewArrival} onChange={(e) => updateParams({ isNewArrival: e.target.value })}>
                        ...
                        {products.map((p) => (
                            <div key={p._id} className="bg-white rounded-xl border border-gray-200 p-4">
                                <div className="flex items-start gap-3">
                                    {p.images?.[0]?.url ? (
                                        <img src={p.images[0].url} alt={p.images[0].altText || p.name} className="h-16 w-16 rounded-lg object-cover bg-gray-100 flex-shrink-0" />
                                    ) : (
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-xs text-gray-400 mt-0.5">{p.productCode} &middot; {p.category?.name || '—'}</p>
